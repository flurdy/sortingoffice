...
pub async fn convert_to_backup(
    State(state): State<AppState>,
    Path(id): Path<i32>,
    headers: HeaderMap,
) -> Html<String> {
    ...
    if state.config.is_database_read_only(&current_db_id) {
        let error_msg = get_translation(&state, &locale, "error-read-only-mode").await;
        return Html(error_msg);
    }

    match db::convert_domain_to_backup(&pool, id) {
        Ok(backup) => {
        ...
        Err(e) => {
            error!("Failed to convert domain to backup: {:?}", e);
            let error_msg = get_translation(&state, &locale, "domains-convert-error").await;
