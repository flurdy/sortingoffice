...
{
    timeout(timeout_duration, test_fn).await
...
async fn test_homepage_loads() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        assert!(current_url.as_str().contains("host.docker.internal:3000"));
        
        // Check for basic page elements
        let title_text = client.title().await?;
        println!("Page title: '{}'", title_text);
        assert!(title_text.len() > 0, "Title should not be empty, got: '{}'", title_text);
...
async fn test_navigation_menu() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        let nav_elements = client.find_all(fantoccini::Locator::Css("nav, .nav, .navbar, .menu")).await?;
        assert!(!nav_elements.is_empty() || client.find(fantoccini::Locator::Css("a")).await.is_ok());
...
async fn test_error_pages() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        // Test 404 page
        client.goto("http://host.docker.internal:3000/nonexistent-page").await?;
        
        // Should show some error content
        let body = client.find(fantoccini::Locator::Css("body")).await?;
        let body_text = body.text().await?;
        assert!(body_text.contains("404") || body_text.contains("Not Found") || body_text.contains("Error"));
...
async fn test_page_titles() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        let pages = ["/", "/dashboard", "/domains", "/users", "/aliases", "/stats"];
        
        for page in pages {
            client.goto(&format!("http://host.docker.internal:3000{}", page)).await?;
            let title_text = client.title().await?;
...
async fn test_form_validation() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        if !forms.is_empty() {
            ...
            let inputs = client.find_all(fantoccini::Locator::Css("input, textarea, select")).await?;
...
async fn test_cross_browser_compatibility() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        let viewports = [(1920, 1080), (1366, 768), (1024, 768), (768, 1024), (375, 667)];
...
async fn test_users_list_page() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        client.goto("http://host.docker.internal:3000/users").await?;
...
async fn test_aliases_list_page() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        client.goto("http://host.docker.internal:3000/aliases").await?;
...
async fn test_stats_page() -> Result<()> {
    ...
    run_test_with_timeout(async {
        ...
        client.goto("http://host.docker.internal:3000/stats").await?;
        
        // Check for stats page content
        let body = client.find(fantoccini::Locator::Css("body")).await?;
        let body_text = body.text().await?;
        assert!(body_text.contains("Statistics") || body_text.contains("stats") || body_text.contains("Statistics"));
