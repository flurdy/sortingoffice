...
run_integration_tests() {
    ...
    start_time=$(date +%s)
    if cargo test --test integration --test handlers --test testcontainers_test --test cross_database_domain_tests --test domain_conversion_tests -- --test-threads=$TEST_THREADS; then
        end_time=$(date +%s)
        duration=$((end_time - start_time))
