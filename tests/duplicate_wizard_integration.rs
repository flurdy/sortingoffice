...
async fn test_duplicate_wizard_form_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let minimal_form_data =
        "source_domain=example.com&new_domain=test-example.com&transport=virtual";
...
async fn test_duplicate_wizard_form_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let long_form_data = format!(
        "source_domain=example.com&new_domain={}.com&transport=virtual",
        long_domain
    );
    ...
    let special_transport_form_data =
        "source_domain=example.com&new_domain=test.com&transport=smtp:[2001:db8::1]:587&enabled=on";
    let special_form: DuplicateDomainForm =
        serde_urlencoded::from_str(special_transport_form_data)?;
...
#[tokio::test]
async fn test_duplicate_wizard_form_confirmation_scenarios(
) -> Result<(), Box<dyn std::error::Error>> {
    // Test different confirmation scenarios

    // Test confirmation form
    let confirm_form_data =
        "source_domain=example.com&new_domain=test.com&transport=virtual&confirmed=true";
    ...
    let cancel_form_data =
        "source_domain=example.com&new_domain=test.com&transport=virtual&confirmed=false";
...
async fn test_duplicate_wizard_domain_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let uppercase_form_data =
        "source_domain=example.com&new_domain=INVALID-DOMAIN.com&transport=virtual";
    ...
    let consecutive_dots_form_data =
        "source_domain=example.com&new_domain=test..domain.com&transport=virtual";
    let consecutive_dots_form: DuplicateDomainForm =
        serde_urlencoded::from_str(consecutive_dots_form_data)?;
    ...
    let invalid_chars_form_data =
        "source_domain=example.com&new_domain=test@domain.com&transport=virtual";
    let invalid_chars_form: DuplicateDomainForm =
        serde_urlencoded::from_str(invalid_chars_form_data)?;
    ...
    let start_hyphen_form_data =
        "source_domain=example.com&new_domain=-invalid.com&transport=virtual";
    let start_hyphen_form: DuplicateDomainForm =
        serde_urlencoded::from_str(start_hyphen_form_data)?;
