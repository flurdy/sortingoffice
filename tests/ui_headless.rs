...
async fn wait_for_selenium_ready(port: u16, max_wait: Duration) -> Result<()> {
    ...
    loop {
        ...
        if start.elapsed() > max_wait {
            return Err(anyhow::anyhow!("Timed out waiting for Selenium to be ready on port {}", port));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}

async fn setup_driver<'a>(docker: &'a clients::Cli) -> Result<(WebDriver, Container<'a, GenericImage>, u16)> {
    ...
    if let Err(e) = wait_for_selenium_ready(port, Duration::from_secs(20)).await {
        println!("Selenium container failed to become ready. Container ID: {} | Image: {:?}", selenium.id(), selenium.image());
    ...
    let driver = timeout(Duration::from_secs(10), WebDriver::new(&format!("http://localhost:{}", port), caps)).await??;
...
async fn authenticate_driver(driver: &WebDriver, app_port: u16) -> Result<()> {
    ...
    let username_field = timeout10s!(driver.find(By::Css("input[name='id']")), "Find username field");
    ...
    let password_field = timeout10s!(driver.find(By::Css("input[name='password']")), "Find password field");
    ...
    let submit_button = timeout10s!(driver.find(By::XPath("//button[@type='submit' and contains(text(), 'Sign in')]")), "Find submit button");
    ...
    println!("Button enabled: {}, displayed: {}", is_enabled, is_displayed);
    
    if is_enabled && is_displayed {
        timeout10s!(submit_button.click(), "Click submit button");
        println!("Form submitted");
    } else {
        return Err(anyhow::anyhow!("Submit button is not clickable: enabled={}, displayed={}", is_enabled, is_displayed));
    }
    ...
    if current_url.as_str().contains("/login") {
        return Err(anyhow::anyhow!("Still on login page after authentication attempt"));
    }
...
async fn test_homepage_loads_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        if !page_source.contains("Dashboard") && !page_source.contains("dashboard") {
            return Err(anyhow::anyhow!("Dashboard page does not contain expected content"));
        }
...
async fn test_domains_list_page_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        if !page_source.contains("Domains") && !page_source.contains("domains") {
            return Err(anyhow::anyhow!("Domains page does not contain expected content"));
        }
...
async fn test_users_list_page_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        if !page_source.contains("Users") && !page_source.contains("users") {
            return Err(anyhow::anyhow!("Users page does not contain expected content"));
        }
...
async fn test_clients_list_page_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        if !page_source.contains("Clients") && !page_source.contains("clients") {
            return Err(anyhow::anyhow!("Clients page does not contain expected content"));
        }
...
async fn test_responsive_design_headless() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            timeout10s!(driver.set_window_rect(0, 0, 1920, 1080), "set window rect to desktop");
            timeout10s!(driver.get(&format!("http://172.17.0.1:{}", app_port)), "navigate to homepage for desktop viewport");

            // Test mobile viewport
            timeout10s!(driver.set_window_rect(0, 0, 375, 667), "set window rect to mobile");
            timeout10s!(driver.get(&format!("http://172.17.0.1:{}", app_port)), "navigate to homepage for mobile viewport");

            // Both should load without errors
            let current_url = timeout10s!(driver.current_url(), "get current url after responsive nav");
...
async fn test_error_pages_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        let page_source = timeout10s!(driver.source(), "Get page source");
        if !page_source.contains("404") && !page_source.contains("Not Found") && !page_source.contains("Error") {
            return Err(anyhow::anyhow!("404 page does not contain expected error content"));
        }
...
async fn test_form_validation_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        let inputs = timeout10s!(driver.find_all(By::Css("input, textarea, select")), "Find input elements");
        if inputs.is_empty() {
...
async fn test_navigation_menu_headless() -> Result<()> {
    run_test_with_timeout(async {
        ...
        let nav_elements = timeout10s!(driver.find_all(By::Css("nav, .nav, .navbar, .menu")), "Find navigation elements");
    ...
    run_test_with_timeout(async {
        ...
        for (width, height) in viewports {
            ...
            if !current_url.as_str().contains("3000") {
                return Err(anyhow::anyhow!("Page should load correctly at {}x{} viewport", width, height));
            }
        }
