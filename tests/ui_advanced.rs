...
async fn wait_for_element(
    ...
    loop {
        match client.find(locator.clone()).await {
            Ok(el) => return Ok(el),
            Err(_) => {
                if start.elapsed() > StdDuration::from_millis(timeout_ms) {
                    return Err(anyhow::anyhow!(
    ...
    run_test_with_timeout(
        async {
            ...
            let submit_button = wait_for_element(
                &client,
                Locator::Css("button[type='submit'], input[type='submit']"),
                5000,
            )
            .await?;
...
async fn test_user_creation_workflow() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let password_input =
                wait_for_element(&client, Locator::Css("input[name='password']"), 5000).await?;
...
async fn test_alias_creation_workflow() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            // Fill out the form
            let mail_input =
                wait_for_element(&client, Locator::Css("input[name='mail']"), 5000).await?;
            mail_input.send_keys("testalias@testdomain.com").await?;

            let destination_input =
                wait_for_element(&client, Locator::Css("input[name='destination']"), 5000).await?;
            destination_input
                .send_keys("testuser@testdomain.com")
                .await?;
            ...
            assert!(
                body_text.contains("testalias@testdomain.com"),
                "Alias should appear in the list after creation"
            );
...
async fn test_form_validation_errors() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let current_url = client.current_url().await?;
            assert!(
                current_url.as_str().contains("/new")
                    || body_text.contains("error")
                    || body_text.contains("required")
            );
...
async fn test_navigation_breadcrumbs() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let breadcrumbs = wait_for_elements(
                &client,
                Locator::Css(".breadcrumb, .breadcrumbs, nav[aria-label='breadcrumb']"),
                5000,
            )
            .await?;
    ...
    run_test_with_timeout(
        async {
            ...
            if !tables.is_empty() {
                ...
                let sortable_headers = wait_for_elements(
                    &client,
                    Locator::Css("th[data-sort], th.sortable, .sortable"),
                    5000,
                )
                .await?;

                // Check for pagination
                let pagination = wait_for_elements(
                    &client,
                    Locator::Css(".pagination, .pager, nav[aria-label='pagination']"),
                    5000,
                )
                .await?;
...
async fn test_search_functionality() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let search_inputs = wait_for_elements(
                &client,
                Locator::Css(
            ...
            if !search_inputs.is_empty() {
                ...
                let search_button = wait_for_element(
                    &client,
                    Locator::Css("button[type='submit'], .search-btn"),
                    5000,
                )
                .await?;
...
async fn test_modal_dialogs() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let modal_triggers = wait_for_elements(
                &client,
                Locator::Css("button[data-toggle='modal'], button[data-target], .modal-trigger"),
                5000,
            )
            .await?;
            ...
            if !modal_triggers.is_empty() {
                ...
                let modals =
                    wait_for_elements(&client, Locator::Css(".modal, [role='dialog']"), 5000)
                        .await?;
                assert!(
                    !modals.is_empty(),
                    "Modal should appear after clicking trigger"
                );
            }
...
async fn test_performance_metrics() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            assert!(
                nav_time.as_millis() < 3000,
                "Navigation took too long: {}ms",
                nav_time.as_millis()
            );
    ...
    run_test_with_timeout(
        async {
            ...
            for url in error_urls {
                ...
                // Debug: Print the page content for this URL
                println!("Error page content for {}: {}", url, body_text);

                // Should contain error indicators (case-insensitive for 'not found')
                let body_text_lower = body_text.to_lowercase();
                assert!(
                    body_text.contains("404")
                        || body_text.contains("Error")
                        || body_text.contains("error")
                        || body_text.contains("Not Found")
                        || body_text_lower.contains("not found"),
                    "Error page should show error content for URL: {} (got: {})",
                    url,
                    body_text
                );
            }
...
#[tokio::test]
#[ignore = "Accessibility tests take too long - disabled for now"]
async fn test_accessibility_features() -> Result<()> {
    ...
    run_test_with_timeout(
        async {
            ...
            let elements_with_aria = wait_for_elements(
                &client,
                Locator::Css("[aria-label], [aria-labelledby], [role]"),
                5000,
            )
            .await?;
            ...
            let images_with_alt =
                wait_for_elements(&client, Locator::Css("img[alt]"), 5000).await?;

            // Check for form labels
            let _form_inputs =
                wait_for_elements(&client, Locator::Css("input, textarea, select"), 5000).await?;
            let inputs_with_labels = wait_for_elements(
                &client,
                Locator::Css("input[id], textarea[id], select[id]"),
                5000,
            )
            .await?;
            ...
            assert!(
                elements_with_aria.len() > 0
                    || images_with_alt.len() > 0
                    || inputs_with_labels.len() > 0
            );
